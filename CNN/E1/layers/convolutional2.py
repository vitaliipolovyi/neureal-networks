...
class Convolutional2:
    def __init__(self, name, k_filters=16, stride=1, size=3, activation=None):
        ...
        self.leaky_relu = np.vectorize(utils.relu)
    ...
    def forward(self, image):
        ...
        #print(image.shape)
        input_dimension_x = image.shape[2]                                                # input dimension
        input_dimension_y = image.shape[1]                                                # input dimension
        output_dimension_x = int((input_dimension_x - self.size) / self.stride) + 1         # output dimension
        output_dimension_y = int((input_dimension_y - self.size) / self.stride) + 1         # output dimension
        #print(input_dimension_x, input_dimension_y, output_dimension_x, output_dimension_y)
        out = np.zeros((self.filters.shape[0], output_dimension_y, output_dimension_x))     # create the matrix to hold the
        ...
        for f in range(self.filters.shape[0]):              # convolve each filter over the image,
            ...
            while tmp_y + self.size <= input_dimension_y:
                tmp_x = out_x = 0
                while tmp_x + self.size <= input_dimension_x:
