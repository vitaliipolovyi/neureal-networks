...
class Network:
    ...
    def build_model(self, dataset_name):
        if dataset_name == 'mnist':
            self.add_layer(Convolutional(name='conv1', k_filters=8, stride=2, size=3, activation='relu'))
            self.add_layer(Convolutional(name='conv2', k_filters=8, stride=2, size=3, activation='relu'))
            self.add_layer(Dense(name='dense', nodes=8 * 6 * 6, num_classes=10))
        else:
            self.add_layer(Convolutional(name='conv1', k_filters=32, stride=1, size=3, activation='relu'))
            self.add_layer(Convolutional(name='conv2', k_filters=32, stride=1, size=3, activation='relu'))
            self.add_layer(MaxPooling(name='pool1', stride=2, size=2))
            self.add_layer(Convolutional(name='conv3', k_filters=64, stride=1, size=3, activation='relu'))
            self.add_layer(Convolutional(name='conv4', k_filters=64, stride=1, size=3, activation='relu'))
            self.add_layer(MaxPooling(name='pool2', stride=2, size=2))
    ...
    def train(self, dataset, num_epochs, learning_rate, validate, regularization, plot_weights, verbose):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for i in range(len(dataset['train_images'])):
                ...
                learning_rate = lr_update(learning_rate, iteration=i)     # learning rate decay
