...
def train_network(X, y, layers, lr=0.01, epochs=200):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch + 1}/{epochs} - Loss: {average_loss:.4f} - Accuracy: {accuracy:.2f}% - Time: {time_epoch:.2f}s")
        
    time_training = time.time() - start
    print(f"Calculations time - -time_training/60:.2f}m")
...
# %%
for X_test_sample in X_test[:10]:
    predictions = predict(X_test_sample, layers)
    print(predictions)
    print(y_labels_names[np.argmax(predictions)])
    plt.imshow(X_test_sample / 255., cmap='gray')
    plt.show()
